...
const HeroSection = () => {
  ...
  const featuresMovies = trendingMovies.slice(0, 5);

  useEffect(() => {
    if (loading || featuresMovies.length === 0) return;

    const interval = setInterval(() => {
      setIsTransitioning(true);
      setTimeout(() => {
        setCurrentSlide((prev) => (prev + 1) % featuresMovies.length);
        setIsTransitioning(false);
      }, 400);
    }, 7000);

    return () => clearInterval(interval);
  }, [loading, featuresMovies.length]);

  if (loading || featuresMovies.length === 0) {
    return (
  ...
  const currentMovie = featuresMovies[currentSlide];
      ...
      <div
        ...
        style={{
          backgroundImage: currentMovie?.backdrop_path
            ? `url(${getImageURL(currentMovie.backdrop_path)})`
            : "none",
        }}
      >
        {/* Gradient overlay */}
            ...
            {featuresMovies.map((_, index) => {
              return (
                <button
                  ...
                  className={`h-1.5 rounded-full transition-all ${currentSlide === index ? "w-8 bg-purple-500" : "w-4 bg-neutral-600/50"}`}
                ></button>
              );
            })}
            {/* Conditional Rendering */}
